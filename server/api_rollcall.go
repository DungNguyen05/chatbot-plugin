...
func (p *Plugin) handleAPICheckIn(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": "Unable to find your employee record in the ERP system. Please contact your administrator to ensure your Mattermost account is linked to your employee profile.",
		})
		return
	}
	...
	if erpErr != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"message": "There was an issue recording your check-in in the ERP system. An administrator has been notified.",
		})
		return
	}
	...
	c.JSON(http.StatusOK, CheckInResponse{
		Success: true,
		Message: fmt.Sprintf("Your check-in has been recorded in the ERP system at %s!", formattedTime),
		Time:    formattedTime,
	})
}
...
func (p *Plugin) handleAPICheckOut(c *gin.Context) {
	...
	if erpErr != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"message": "There was an issue recording your check-out in the ERP system. An administrator has been notified.",
		})
		return
	}
	...
	c.JSON(http.StatusOK, CheckInResponse{
		Success: true,
		Message: fmt.Sprintf("Your check-out has been recorded in the ERP system at %s!", formattedTime),
		Time:    formattedTime,
	})
}
...
func (p *Plugin) handleAPIAbsent(c *gin.Context) {
	...
	if absenceErr != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"message": "There was an issue recording your absence in the ERP system. An administrator has been notified.",
		})
		return
	}
	...
	c.JSON(http.StatusOK, CheckInResponse{
		Success: true,
		Message: fmt.Sprintf("Your absence has been recorded for %s with reason: \"%s\"", recordedDate, reason),
		Time:    recordedDate,
	})
}
